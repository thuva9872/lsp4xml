...
public class APIFactory extends AbstractFactory {
        ...
        if (Objects.nonNull(children)) {
            for (DOMNode node : children) {
                if (node.getNodeName().equalsIgnoreCase(Constant.RESOURCE)) {
                    ...
                    APIHandlers apiHandler = createAPIHandler(node);
    ...
    public APIHandlers createAPIHandler(DOMNode node) {
        ...
        if (children != null && !children.isEmpty()) {
            for (DOMNode childNode : children) {
                if (childNode.getNodeName().equalsIgnoreCase(Constant.HANDLER)) {
                    APIHandlersHandler handler = createAPIHandlerHandler(childNode);
    ...
    private APIHandlersHandler createAPIHandlerHandler(DOMNode childNode) {
